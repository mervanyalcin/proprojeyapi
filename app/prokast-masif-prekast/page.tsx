...
const page = () => {
    ...
    const getAllProjects = useCallback(async (id: string) => {
        try {
            const response = await axios.post("/api/projects/by-brandings", { id })
                    ...
                    <div className="grid grid-cols-3 h-auto gap-x-4 gap-y-10 mt-8">
                        {
                            allProjects.map((project, index) => (
                                <Link href={`/prodekor/${project.id}`} key={index}>
                                    <div className="min-h-[300px] w-full relative group cursor-pointer flex items-center justify-center flex-col">
                                        ...
                                        <Image alt='' src={`/uploads/${project.images}`} width={500} height={500} className='object-cover h-96 w-96 ' />
                                        ...
                                        <p className=' font-bold bg-themeColorOne w-full p-2.5 text-themeColorThird uppercase'> {project.name} </p>
                                    </div>
                                </Link>
                            ))
                        }
